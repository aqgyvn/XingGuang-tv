...
| 分类和搜索连续分页 | 已对齐 | 分类网格和搜索结果到达末项时请求下一页，按影片 ID 去重并在服务端页数结束时停止。 |
...
| 定时停止、循环、片头片尾跳过 | 已对齐 | 点播支持单集循环和按历史保存的片头/片尾；点播、直播、本地媒体均支持 5/15/30/60 分钟定时暂停、延长和取消。 |
| 画面比例、选集倒序 | iOS 等效实现 | 提供原始、16:9、4:3、填充、裁剪；点播比例和倒序写入 Android 兼容历史字段，直播比例独立保存。 |
| 画面镜像、跨线路自动连播 | 不在本轮范围 | Android 当前移动端矩阵中的核心播放流程不依赖这两项，iOS 未添加额外开关。 |
