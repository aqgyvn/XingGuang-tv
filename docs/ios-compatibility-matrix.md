...
| --- | --- | --- |
| 点播配置、直播配置 | 已对齐 | 设置主页提供独立入口；编辑页支持 URL、配置文件和二维码。 |
| 点播主页、直播主页 | iOS 等效实现 | 点播和直播配置卡均提供主页图标；点播只允许配置声明可切换的站点，选择会在重启后恢复；直播优先按 URL 恢复默认来源并兼容旧名称偏好。 |
| 配置历史与切换 | 已对齐 | 点播和直播配置卡均提供独立历史入口；复用 `config` 表按类型显示、重新加载和删除非当前记录，当前配置受删除保护，备份导出包含全部记录。 |
| 播放核心 | iOS 等效实现 | MPV / MDK / AVPlayer 分段选择，无自动模式。 |
| 无痕模式 | 已对齐 | 开启后不写播放历史。 |
| 直播线路自动选择 | 已对齐 | 控制失败后的备用线路切换。 |
| 常规倍速、长按倍速 | iOS 等效实现 | 常规倍速作为 iOS 播放默认值；长按倍速范围为 `2x` 至 `5x`，与 Android `speed` 备份字段一一对应。 |
| 默认与直播画面比例 | 已对齐 | 播放器设置分别保存点播/本地媒体默认比例和直播比例，并导入导出 Android `scale/scale_live`。 |
| 字幕大小、字幕位置、弹幕加载与显示 | 已对齐 | 播放器设置与播放页均提供对应控制；关闭弹幕加载会取消当前弹幕任务并阻止后续自动或手动加载。 |
| 图片尺寸 | iOS 等效实现 | 设置页提供小、中、大、特大四档，点播页立即按所选最小海报宽度重排，并写入备份偏好。 |
| 备份与恢复 | 已对齐 | 支持 Android 字段 JSON、`.bk.gz`、完整校验和事务替换。 |
| 壁纸 | 不在本轮范围 | iOS 固定使用云白响应式界面。 |
...
| 广告主机拦截 | iOS 等效实现 | 播放器设置可开关 App、JavaScript HTTP 和网页嗅探的主机拦截；不承诺拦截 MPV、MDK、AVPlayer 内部媒体请求。 |
