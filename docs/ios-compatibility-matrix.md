...
| 本地媒体文件 | iOS 等效实现 | 通过系统 Files 选择器导入缓存后，交给 AVPlayer/VLC 播放。 |
...
| Android 投屏设备发现与控制 | iOS 等效实现 | AVPlayer 使用系统 AirPlay；不实现 Android DLNA/接收端协议，VLC 路径不承诺 AirPlay。 |
...
| 广告规则 | iOS 等效实现 | 覆盖 App 请求和 WKWebView；AVPlayer/VLC 内部媒体请求无法保证完全复现。 |
...
| --- | --- | --- |
| 多播放核心 | iOS 等效实现 | 提供自动、AVPlayer、VLC；替代 Android EXO/IJK/MPV。 |
| HLS、MP4/MOV | 已对齐 | 默认使用 AVPlayer。 |
| RTSP、RTMP、MKV、FLV、WebM、DASH | iOS 等效实现 | 由 MobileVLCKit 播放。 |
| 自动选核与格式失败回退 | 已对齐 | 只对格式/解码错误从 AVPlayer 回退 VLC 一次。 |
| 播放、暂停、跳转、倍速 | 已对齐 | 点播、直播和本地媒体共用 PlayerSession。 |
| 进度、线路、集数和倍速恢复 | 已对齐 | 通过 GRDB history/track 语义保存。 |
| 内嵌音轨、视频轨、字幕轨 | 已对齐 | AVPlayer 支持轨道选择；VLC 当前不暴露轨道选择。 |
...
| 画中画 | iOS 等效实现 | AVPlayer 使用系统 PiP；VLC 路径不提供。 |
| AirPlay | iOS 等效实现 | AVPlayer 使用系统外部播放；VLC 路径不提供。 |
...
| 强制软/硬解、隧道模式、AAC 优先 | 平台限制 | AVPlayer 不公开对应控制；VLC 也不保证与 Android 参数等价。 |
| Widevine、PlayReady、CENC ClearKey | 平台限制 | 返回专门 DRM 错误；不进行无法验证的回退。 |
| HLS AES 等系统原生加密 | iOS 等效实现 | 交给 AVPlayer/VLC 原生支持范围处理。 |
...
| 播放核心 | iOS 等效实现 | 自动 / AVPlayer / VLC 分段选择。 |
| 无痕模式 | 已对齐 | 开启后不写播放历史。 |
...
| 渲染方式、Libass、系统字幕样式 | 平台限制 | 使用 AVPlayer/VLC 与 App 字幕覆盖层，无 Android 渲染器等价开关。 |
...
- TrollStore 真机必须验证配置与二维码、本地文件、真实点播/直播、VLC 格式、横竖屏、后台恢复、画中画及 AirPlay。
