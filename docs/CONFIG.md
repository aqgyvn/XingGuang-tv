...
Vod 配置为一个 JSON 对象，作为应用程序的主要配置入口。配置可通过 URL、本地路径或直接粘贴字符串的方式加载。
...
| `parses`    | `array<Parse>`  | 视频解析规则清单。详见 [parses](#parses--解析规则)。                           |
| `lives`     | `array<Live>`   | 直播来源清单。详见 [lives — 直播来源](#lives--直播来源)。                        |
| `doh`       | `array<Doh>`    | DNS over HTTPS 设置清单。详见 [doh](#doh--dns-over-https)。            |
| `proxy`     | `array<Proxy>`  | 代理服务器设置清单。详见 [proxy](#proxy--代理服务器)。                           |
| `rules`     | `array<Rule>`   | 网络拦截规则清单。详见 [rules](#rules--网络拦截规则)。                           |
...
| `ext`         | `string`        | 额外扩充数据，传入爬虫 `init()` 使用。可为字符串、JSON 对象或路径。  |
...
| `timeout`     | `integer`       | 请求逾时秒数，覆盖默认值。                             |
| `searchable`  | `integer`       | 搜索开关。`0`=停用，`1`=激活（默认）。                   |
| `changeable`  | `integer`       | 线路切换开关。`0`=停用，`1`=激活（默认）。                 |
| `quickSearch` | `integer`       | 快速搜索开关。`0`=停用，`1`=激活。                     |
| `indexs`      | `integer`       | 索引模式旗标。`1` 表示此来源作为索引来源使用。                 |
| `categories`  | `array<string>` | 显示的分类名称白名单，仅显示清单中的分类。                     |
| `header`      | `object`        | 此来源请求时附加的 HTTP 请求头，格式为键值对。                 |
...
| `1`    | HTTP URL                              | 直接 GET 请求，回传 JSON 格式，额外支持 Filter 筛选参数（`f=`）。                     |
...
| `ext`        | `Parse.Ext`     | 解析器扩充设置对象。                               |
| `ext.flag`   | `array<string>` | 适用旗标清单，标示此解析器适用的平台（如 `["qq", "iqiyi"]`）。 |
| `ext.header` | `object`        | 解析请求时附加的 HTTP 请求头，格式为键值对。                 |
...
| `ext`      | `string`       | 传入直播爬虫的额外扩充数据。                                                                                      |
| `jar`      | `string`       | 此直播来源使用的 Spider JAR 路径/URL。                                                                         |
| `click`    | `string`       | 点击拦截处理 URL 或规则。                                                                                     |
| `logo`     | `string`       | 频道默认 Logo 图片 URL，支持 `{id}`、`{name}`、`{logo}` 变量替换。                                                  |
| `epg`      | `string`       | EPG 节目表 URL，多个以逗号分隔。支持 `{id}`、`{name}`、`{epg}` 变量替换；含 `xml`/`gz` 的条目作为 XMLTV 来源，含 `{` 的条目作为 API 范本。 |
| `ua`       | `string`       | 默认 User-Agent 字符串。                                                                                   |
| `origin`   | `string`       | 请求 `Origin` 请求头值。                                                                                    |
| `referer`  | `string`       | 请求 `Referer` 请求头值。                                                                                   |
| `timeZone` | `string`       | 时区设置，用于 EPG 时间显示（如 `"Asia/Taipei"`）。                                                                |
| `timeout`  | `integer`      | 请求逾时秒数。                                                                                             |
| `header`   | `object`       | 此来源请求时附加的 HTTP 请求头，格式为键值对。                                                                           |
| `catchup`  | `Catchup`      | 回看/时移设置，作为此来源所有频道的默认值。详见 [catchup](#catchup--回看时移)。                                                 |
...
|-----------|-----------------|--------------------------------------------------------------------------------------------------------------------|
| `name`    | `string`        | 频道显示名称。                                                                                                            |
| `urls`    | `array<string>` | 频道播放 URL 清单，支持多个备用线路，依序尝试。每条 URL 可附加 `$线路名称` 后缀指定显示名称（如 `"http://cdn1.example.com/hbo.m3u8$CDN1"`）；省略时自动显示为「线路 N」。 |
...
| `origin`  | `string`        | 请求 `Origin` 请求头值，覆盖来源默认值。                                                                                           |
| `referer` | `string`        | 请求 `Referer` 请求头值，覆盖来源默认值。                                                                                          |
| `tvgId`   | `string`        | TVG 格式 EPG 频道 ID。                                                                                                  |
| `tvgName` | `string`        | TVG 格式 EPG 频道名称。                                                                                                   |
| `header`  | `object`        | 此频道请求的额外 HTTP 请求头，格式为键值对。                                                                                           |
| `parse`   | `integer`       | 是否需要解析此频道 URL。`0`=不解析，`1`=解析。                                                                                      |
| `catchup` | `Catchup`       | 此频道的回看/时移设置，覆盖分组及来源默认值。详见 [catchup](#catchup--回看时移)。                                                               |
...
设置加密 DNS 解析服务器，保护 DNS 查找隐私并防止污染。仅 Vod 配置支持。

| 字段     | 类型              | 说明                                                    |
|--------|-----------------|-------------------------------------------------------|
| `name` | `string`        | 服务器显示名称。                                              |
| `url`  | `string`        | DoH 查找端点 URL（如 `https://dns.google/dns-query`）。       |
| `ips`  | `array<string>` | 服务器本身的 IP 地址清单，用于 Bootstrap 解析，避免 DoH 服务器本身需要 DNS 查找。 |
...
| `hosts` | `array<string>` | 适用此代理的主机名称清单，支持正则表达式。靠前的规则优先匹配。 |
| `urls`  | `array<string>` | 代理服务器 URL 清单（多个时依序尝试）。          |
...
| `regex`   | `array<string>` | 用于截取播放 URL 的正则表达式清单，符合者视为有效的媒体 URL。             |
| `script`  | `array<string>` | 在 WebView 中运行的 JavaScript 代码清单，用于自动点击、关闭广告等操作。 |
| `exclude` | `array<string>` | 排除清单，符合此清单的 URL 不触发 `regex` 截取。                 |
...
针对特定主机，在 HTTP 响应中注入自定义请求头（通常用于解除 CORS 限制）。

| 字段       | 类型       | 说明                            |
|----------|----------|-------------------------------|
| `host`   | `string` | 目标主机名称（不含协议，如 `example.com`）。 |
| `header` | `object` | 要注入的 HTTP 请求头对象，格式为键值对。        |
...
覆盖特定主机名称的 DNS 解析结果，可用于 CDN 调度或绕过封锁。支持通配符 `*`。
...
| `regex`   | `string` | 判断此回看设置是否适用于当前 URL 的比对条件（子字符串或正则表达式）。未设置时只要 `source` 非空即激活；设置后只有 URL 符合此条件才激活回看。                         |
| `source`  | `string` | 时移 URL 范本，**非空时才激活回看功能**。支持 `{(b)格式}`（开始时间）、`{(e)格式}`（结束时间）、`{utc:}`（开始 Unix 秒）、`{utcend:}`（结束 Unix 秒）。 |
| `replace` | `string` | 逗号分隔的替换对（`原字符串,新字符串`），在组合时移 URL 前先对原始 URL 运行替换。                                                           |
...
接口须回传 JSON 数组，每个元素为一个弹幕来源对象。

### 自动搜索条件

- 设置中「弹幕加载」已打开（`DanmakuSetting.isLoad()`）
- 设置中「自动加载」已打开（`DanmakuSetting.isAuto()`）
