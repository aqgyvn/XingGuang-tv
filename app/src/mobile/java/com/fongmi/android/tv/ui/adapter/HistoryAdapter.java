...
public class HistoryAdapter extends BaseDiffAdapter<History, HistoryAdapter.ViewHolder> {
    ...
    private void setTime(ViewHolder holder, History item) {
        long position = item.getPosition();
        if (position <= 0) {
            holder.binding.time.setVisibility(View.GONE);
            return;
        }
        String time = DateUtils.formatElapsedTime(position / 1000);
        long duration = item.getDuration();
        if (duration > 0) time = time + "/" + DateUtils.formatElapsedTime(duration / 1000);
