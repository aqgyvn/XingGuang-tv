...
public class Setting {
    ...
    public static final int PLAYER_MPV = 2;
    ...
    public static int getPlayer() {
        return Math.min(Math.max(Prefers.getInt("player_engine", PLAYER_EXO), PLAYER_EXO), PLAYER_MPV);
    }

    public static void putPlayer(int player) {
        Prefers.put("player_engine", Math.min(Math.max(player, PLAYER_EXO), PLAYER_MPV));
    }

    public static boolean isExo() {
        return getPlayer() == PLAYER_EXO;
    }

    public static boolean isIjk() {
        return getPlayer() == PLAYER_IJK;
    }

    public static boolean isMpv() {
        return getPlayer() == PLAYER_MPV;
    }
    ...
    public static boolean isVideoPurify() {
        return Prefers.getBoolean("video_purify", Prefers.getBoolean("adblock", true));
    }

    public static void putVideoPurify(boolean videoPurify) {
        Prefers.put("video_purify", videoPurify);
    }

    public static boolean isAdHostBlock() {
        return Prefers.getBoolean("ad_host_block", Prefers.getBoolean("adblock", true));
    }

    public static void putAdHostBlock(boolean adHostBlock) {
        Prefers.put("ad_host_block", adHostBlock);
    }

    public static boolean isAdblock() {
        return isVideoPurify() && isAdHostBlock();
    }

    public static void putAdblock(boolean adblock) {
        putVideoPurify(adblock);
        putAdHostBlock(adblock);
    }
